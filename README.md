...
This program is designed to run with large point clouds where each point is assigned to a Flying Light Speck, a process.  If running on a laptop/server with a limited number of cores, set the 'NUMBER_OF_FLSS' and 'SAMPLE_SIZE' to a small value.  As a general guideline, their values should not exceed four times the number of physical cores.
...
## Outring
The Outring is a synthetically generated point cloud.  Set the value of `R` in `config.py` to a large value, say 1000, to generate sparse point clouds.  Such point clouds have known optimal solutions.  One may evaluate the implementation of the alternative algorithms to see if they can compute this optimal solution.

To run the Outring, set `TEST_ENABLED` to True and `SHAPE` to `test` to run an Outring experiment.  It will generate the synthetic point cloud in memory and invoke the algorithm specified by the parameter `H` in `config.py`.  The possible values of `H` are:  canf, simpler, rs, vns.
